...
class Worker(typing.Protocol):
    ...
    async def chat_completion(
        self, messages: ChatCompletionMessages, args: GenerateArgs | None = None
    ) -> str:
...
async def _retry_post(
    client: httpx.AsyncClient, url: str, json_data: dict, http_config: HTTPConfig
) -> httpx.Response:
    for attempt in range(http_config.retry_limit):
        try:
            ...
            logger.warning(
                f"HTTP request error on attempt {attempt + 1}/{http_config.retry_limit}: {e}"
            )
        except httpx.HTTPStatusError as e:
            if e.response.status_code / 100 == 4:
                logger.debug(
                    f"Client error {e.response.status_code}: {e.response.text}, ignore retries."
                )
                raise
            logger.warning(
                f"HTTP status error on attempt {attempt + 1}/{http_config.retry_limit}: {e}"
            )
    ...
    raise RuntimeError(
        f"Failed to POST to {url} after {http_config.retry_limit} attempts"
    )
...
class VLLMWorker(Worker):
    ...
    async def close(self) -> None:
        ...
        logger.debug(
            "unloading lora adapter %s, stack %s",
            self._worker_id,
    ...
    async def chat_completion(
        self, messages: list[dict[str, str]], args: GenerateArgs | None = None
    ) -> str:
        if args is None:
            args = GenerateArgs()
        prompt: str = await self._chat_template.apply_chat_template(
            messages, add_generation_token=True
        )
        response: str = ""
...
class VLLMCluster(Cluster):
    def __init__(self, config: VLLMClusterConfig, base_cfg: "Config"):
        ...
        self._http_client = httpx.AsyncClient(
            timeout=httpx.Timeout(read=1800, connect=60, write=60, pool=5),
            limits=httpx.Limits(
                max_connections=sys.maxsize, max_keepalive_connections=sys.maxsize
            ),
        )
    ...
    async def start(self):
        async with asyncio.TaskGroup() as tg:
            tasks = [
                tg.create_task(self._health_check(addr))
                for addr in self._config.addresses
            ]
    ...
    async def new_worker(self, states: dict[str, torch.Tensor]) -> "Worker":
        ...
        await loop.run_in_executor(
            ...
            _save_lora_model,
            worker_dir,
            self._base_config.lora_r,
            self._base_config.target_modules,
            states,
        )
        ...
        logger.info(
            f"Assigning new worker {worker_id} to address {addr} (total workers on this addr: {counter})"
        )
...
class _ChatTemplate:
    ...
    def _apply_chat_template(
        self, messages: ChatCompletionMessages, add_generation_token=False
    ) -> str:
        return self._tokenizer.apply_chat_template(
            messages, tokenize=False, add_generation_token=add_generation_token
        )
...
class StandaloneVLLMCluster(Cluster):
    def __init__(self, config: StandaloneVLLMClusterConfig, base_cfg: Config):
        ...
        self._http_client = httpx.AsyncClient(
            timeout=httpx.Timeout(read=1800, connect=5, write=5, pool=5)
        )
        self._pool = ThreadPoolExecutor(max_workers=8)
        self._chat_template = _ChatTemplate(self._base_config.base_model, self._pool)
...
@contextlib.asynccontextmanager
async def connect_cluster(config: Config):
